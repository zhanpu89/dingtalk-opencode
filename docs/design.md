...
handleRobotMessage(raw)
  ...
  │   ├─ 切换项目 → ProjectServerManager.ensureStarted(project)
  │   │              ├─ 已启动 → 复用 baseUrl
...
| `project-server-manager.ts` | 动态启动、复用、健康检查、回收项目级 `opencode serve` |
| `project-context-store.ts` | 持久化钉钉会话当前项目绑定 |

**ProjectServerManager 状态结构**：

```ts
interface ProjectServerInstance {
  projectId: string;
  projectPath: string;
  port: number;
  baseUrl: string;
