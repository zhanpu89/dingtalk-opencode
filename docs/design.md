...
- 看门狗与 sendMessage 解耦：watchdog 仅监控不中断消息
...
handleRobotMessage(raw)              ← index.ts
  ...
  └─ queue.enqueue(sessionKey, async () => {
       └─ processAIMessage(ctx, sessionKey, message, msg)  ← ai-handler.ts
            ...
            ├─ do {
            │   ├─ sessions.get → sessionId (验证有效性 or createSession)
            │   ├─ new Watchdog(sessionId).start()  # 仅监控，不传 abort signal
            │   ├─ opencode.sendMessage(sessionId, msg)  # 内部 1800s 超时
            │   ├─ watchdog.stop()
...
- **仅监控不中断**：watchdog 不再持有 AbortController，不调用 abort()
- 消息超时控制完全由 `sendMessage` 内部的 1800s 超时负责
- watchdog 检测到的状态（server_down / restart）供 `sendProcessingError` 错误提示和重试逻辑使用

**行为变更**（v1.1.0）：
- 之前 watchdog 与 sendMessage 共享同一 AbortController → 看门狗 3 次健康检查失败会 abort 正在发送的消息
- 分离后 → watchdog 的健康检查使用 `quickHealth()`（5s 短超时），检测结果仅记录状态
- `sendMessage` 使用自己的 `request()` 方法（1800s 超时），不再接收外部 signal
...
每个 tick:
  1. checkHealth()
     └─ opencode.quickHealth()    # 5s 短超时，专为看门狗设计
         ├─ healthy → 重置连续失败计数
         └─ 不健康 → 连续失败++
             └─ ≥3 → state = "server_down"（不再 abort）
  2. checkSession()
     └─ opencode.sessionExists(sessionId)
         ├─ false → state = "restart"（不再 abort）
         ├─ true  → 跳过
...
| 健康检查连续失败 = 3 | state = server_down，**不调用 abort**（v1.1.0 变更）|
| 健康检查异常（throw） | 计入连续失败计数 |
| 使用 quickHealth 而非 health | 调用 opencode.quickHealth()，5s 超时返回结果 |
| session 存在 | state 不变 |
| session 消失 | state = restart，**不调用 abort**（v1.1.0 变更）|
...
| `强制重启` / `暴力重启` | 停止所有 opencode 服务 → 持久化会话 → 执行 restart_bot.sh → 进程退出 |
...
| `watchdog.ts` | 并行健康检查 + session 存活检测，仅监控不中断消息（v1.1.0 与 sendMessage 解耦） |
...
### 3.2 为什么看门狗不再使用 AbortController？（v1.1.0 变更）

v1.0 中，看门狗与 sendMessage 共享同一 AbortController。
问题：看门狗的 `quickHealth()` 使用 5s 短超时，在服务负载高时可能误判健康失败 → 触发 abort → 中断正常消息。

**决策**：看门狗与消息发送的 AbortController 完全分离：
- 看门狗不再持有 AbortController，仅记录状态（server_down / restart）
- `sendMessage` 使用自己的内部 `request()` 方法（1800s 超时）
- 看门狗健康检查失败不再导致消息中断，消息超时完全由 sendMessage 的 1800s 超时控制
- 看门狗检测到的状态仍然会被 `sendProcessingError` 用于更精确的错误提示（如"OpenCode 服务不可用"）
...
| 看门狗 `restart` 状态（session 消失） | 验证 session 不存在后创建新 session，重发消息 | 3 次 |
| `AbortError`（sendMessage 内部 1800s 超时） | 创建新 session，重发消息 | 3 次 |

> **v1.1.0 变更**：之前 AbortError 可能由看门狗触发（共享 AbortController）。现已分离，AbortError 仅来自 sendMessage 内部的 1800s 请求超时。重试次数从原来的"连续 2 次 AbortError 才算触发"改为"每次 AbortError 都触发重试"，与其他条件统一为最多 3 次。
...
| 场景 | 预期行为 |
|------|---------|
| opencode 服务不可用 | 看门狗 3 次检查后 state=server_down → 超时后提示 ❌ OpenCode 服务不可用 |
| 处理中途 session 消失 | 看门狗检测 state=restart → ai-handler 创建新 session 重试 |
| 钉钉 Webhook 过期 | 日志 error，发送简短通知 |
| 网络断连后恢复 | 看门狗检测到恢复，继续等待 |
| 消息队列中前一个任务卡死 | 不影响后续任务（心跳仍运行） |
| SIGTERM/SIGINT | 清理 session-store，断开 WebSocket，正常退出 |
| 看门狗误判健康失败 | 不再 abort 消息，仅记录状态，消息仍由 1800s 超时控制 |
| 强制重启 | 停止所有进程 → flush 会话 → spawn 重启脚本 → exit(0) |
